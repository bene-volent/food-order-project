...
export default function Cart() {
    ...
    const {
        meals, cart, totalQuantity, getOrderAmount, removeItem, clearCart
    } = useOrders()

    const modalRef = useRef(null)

    const cartItems = useMemo(() =>
        meals.filter(meal => cart.get(meal.id))
        , [cart.size])


    function handleClearCart() {
        modalRef.current.close(clearCart)
    }
    ...
    return <aside className="sidebar">
        <div className="cart">
            ...
            <div className="cart-section">
                {totalQuantity === 0 && emptyCartPlaceHolder}
                {totalQuantity !== 0 && <ul>
                    {cartItems.map(item => <CartItem key={item.id} name={item.name} quantity={getOrderAmount(item.id)} price={item.price} removeItem={() => removeItem(item.id)} />)}
                </ul>
                }
            </div>
            {totalQuantity !== 0 &&
                <>
                    ...
                    <button className="cart-confirm" onClick={openCheckout}>Confirm Order</button>
        ...
        <Modal ref={modalRef}>
            ...
            <ul>
                {cartItems.map(item => <CartModalItem key={item.id} name={item.name} quantity={getOrderAmount(item.id)} price={item.price} image={item.image} />)}
            </ul>
            <div className="cart-total" >
                <p>Order Total</p>
                <span>${orderTotal.toFixed(2)}</span>
            </div>
            <button className="cart-confirm" onClick={handleClearCart}>Start New Order</button>
        </Modal>
    </aside>
}